...
def kirsch_hoop_stress(r, theta, sigma_H, sigma_h, wellbore_radius, Pp):
    """Calculate hoop stress around wellbore using Kirsch equations"""
...
def far_field_stress(sigma_H, sigma_h, theta):
    """Calculate far-field stress components"""
    sigma_xx = sigma_H * np.cos(theta)**2 + sigma_h * np.sin(theta)**2
    sigma_yy = sigma_H * np.sin(theta)**2 + sigma_h * np.cos(theta)**2
    sigma_xy = (sigma_H - sigma_h) * np.sin(theta) * np.cos(theta)
...
def plot_stress_vs_depth(depth, sigma_H, sigma_h, Pp, min_depth, max_depth):
    """Plot stress components versus depth"""
    fig = go.Figure()
    ...
    fig.add_shape(type="rect",
        x0=min(np.nanmin(sigma_H), np.nanmin(sigma_h), np.nanmin(Pp)),
        x1=max(np.nanmax(sigma_H), np.nanmax(sigma_h), np.nanmax(Pp)),
...
def calculate_stresses(depth_data, sigma_H_data, sigma_h_data, Pp_data, azimuth_data,
    ...
    """Calculate stresses using finite difference method"""
    ...
    try:
        ...
        valid_idx = ~np.isnan(depth_data) & ~np.isnan(sigma_H_data) & ~np.isnan(sigma_h_data) & ~np.isnan(Pp_data)
        depth_data = depth_data[valid_idx]
        sigma_H_data = sigma_H_data[valid_idx]
        sigma_h_data = sigma_h_data[valid_idx]
        Pp_data = Pp_data[valid_idx]
        azimuth_data = azimuth_data[valid_idx]
...
def create_3d_visualization(X, Y, Z, hoop_stress_fd, wellbore_radius, current_depth, 
    ...
    """Create 3D visualization of stress distribution"""
...
def create_2d_plots(R, Theta, Depth, hoop_stress_fd, sigma_H_3d, sigma_h_3d, Pp_3d, 
    ...
    """Create 2D polar and Cartesian plots of stress distribution"""
...
def create_3d_pipe_visualization(depths, hoop_stress_values, wellbore_radius, color_scale='jet'):
    """
    Create a 3D pipe/borehole visualization color-coded by hoop stress
    
    Args:
        depths (array): Depth values along the wellbore
        hoop_stress_values (array): Hoop stress values at each depth
        wellbore_radius (float): Radius of the wellbore
        color_scale (str): Plotly color scale name
        
    Returns:
        plotly.graph_objects.Figure: 3D pipe visualization figure
    """
    # Create cylindrical coordinates
    theta = np.linspace(0, 2*np.pi, 36)  # 36 points around circumference
    z = depths
    theta_grid, z_grid = np.meshgrid(theta, z)
    
    # Create pipe surface coordinates
    x = wellbore_radius * np.cos(theta_grid)
    y = wellbore_radius * np.sin(theta_grid)
    
    # Repeat stress values for each theta to create full surface colors
    stress_surface = np.tile(hoop_stress_values, (len(theta), 1)).T
    
    fig = go.Figure()
    
    # Add the pipe surface
    fig.add_trace(go.Surface(
        x=x,
        y=y,
        z=z_grid,
        surfacecolor=stress_surface,
    ...
    fig.add_trace(go.Surface(
        x=wellbore_radius * np.cos(theta_grid[:1,:]),
        y=wellbore_radius * np.sin(theta_grid[:1,:]),
        z=np.tile(z[0], (1, len(theta))),
        surfacecolor=np.tile(hoop_stress_values[0], (1, len(theta))),
        colorscale=color_scale,
        showscale=False,
        name='Top'
    ))
    ...
    fig.add_trace(go.Surface(
        x=wellbore_radius * np.cos(theta_grid[:1,:]),
        y=wellbore_radius * np.sin(theta_grid[:1,:]),
        z=np.tile(z[-1], (1, len(theta))),
        surfacecolor=np.tile(hoop_stress_values[-1], (1, len(theta))),
        colorscale=color_scale,
        showscale=False,
        name='Bottom'
    ))
    ...
    # Add direction indicators
    arrow_length = wellbore_radius * 3
    mid_depth = np.mean(z)
    ...
    # Update layout
    fig.update_layout(
        title='3D Wellbore Pipe with Hoop Stress',
        scene=dict(
            xaxis_title='X (ft)',
            yaxis_title='Y (ft)',
            zaxis_title='Depth (ft)',
            ...
            try:
                ...
                if st.button("Run Analysis"):
                    with st.spinner("Calculating stresses..."):
                        ...
                        if results:
                            mid_depth_idx = len(results['depth_range']) // 2
                            current_depth = results['depth_range'][mid_depth_idx]
                            ...
                            with col2:
                                ...
                                st.subheader("3D Wellbore Pipe Visualization")
                                # Get hoop stress at wellbore wall for all depths
                                wall_stress = []
                                for i in range(len(results['sigma_H'])):
                                    stress = kirsch_hoop_stress(
                                        wellbore_radius,
                                        0,  # theta=0 for maximum stress
                                        results['sigma_H'][i],
                                        results['sigma_h'][i],
                                        wellbore_radius,
                                        results['Pp'][i]
                                    )
                                    wall_stress.append(stress)
                                
                                pipe_fig = create_3d_pipe_visualization(
                                    results['depth_range'],
                                    np.array(wall_stress),
                                    wellbore_radius,
                                    'jet'
                                )
                ...
                st.error("Please check that all required curves are present in the LAS file and contain valid data")
        else:
