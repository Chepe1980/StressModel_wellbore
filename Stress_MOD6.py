...
def create_3d_visualization(X, Y, Z, hoop_stress_fd, wellbore_radius, current_depth, current_azimuth, threshold_percent):
    ...
    # Add surface plot for wellbore stress
    fig.add_trace(
        go.Surface(
            x=X[:,:,mid_depth_idx],
            y=Y[:,:,mid_depth_idx],
    ...
    for col in [1, 2]:
        fig.add_trace(
            go.Scatter3d(
                x=x_wall,
                y=y_wall,
                z=np.full_like(x_wall, Z[:,:,mid_depth_idx].min()),
                mode='lines',
        ...
        fig.add_trace(
            go.Scatter3d(
                x=x_wall,
                y=y_wall,
                z=np.full_like(x_wall, Z[:,:,mid_depth_idx].max()),
                mode='lines',
                line=dict(color='black', width=2),
                showlegend=False
            ),
    ...
    # Add North and stress direction indicators
